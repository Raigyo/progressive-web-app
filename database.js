/* Abstraction layer: IndexDB interraction */

function technoDb() {
  return idb.open("techno-store", 2, (upgradeDB) => {
    switch (upgradeDB.oldVersion) {
      case 0:
        upgradeDB.createObjectStore("techno", { autoIncrement: true });
    }
  });
}
...
function getTechno(id) {
  return technoDb().then((db) => {
    return db.transaction("techno").objectStore("techno").get(id);
  });
}
  ...
  return technoDb().then((db) => {
    return db
      ...
      .then((keys) => {
        return Promise.all(
          keys.map((id) =>
            getTechno(id).then((content) => Object.assign({}, { id }, content))
          )
        );
      });
  });
